...
# 🗓️ 2026-03-22 (일)
## 🧩 스프링 부트 & Next.js 연동
실습 깃허브: https://github.com/devjjune/frontend-practice.git 

블로그 정리: https://velog.io/@hjy648012/%EC%8A%A4%ED%94%84%EB%A7%81-%EB%B6%80%ED%8A%B8-Next.js-%EC%97%B0%EB%8F%99
...
# 🗓️ 2026-03-25 (수)
## 🧩 유클리드 호제법이란
정의: 나머지를 이용해서 최대공약수를 계속 줄여가며 구하는 방법

원리: `a = bq + r` => `gcd(a, b) = gcd(b, r)`  
a대신 나머지 r로 바꿔도 최대공약수는 그대로다 

동작 방식:
`gcd(a, b)` → `gcd(b, r)` → `gcd(r, r2)` → ... → `gcd(x, 0)`   
gcd(나눈 수, 나머지)  
마지막에 나머지가 0이 되면, 그때의 x값이 최대공약수이다.
...
자바스크립트에서 함수를 선언할 때는 `function` 키워드를 사용하지만,
ES6부터는 **화살표 함수** 문법을 통해 더 간결하게 함수를 정의할 수 있다.

* `=>` 기호를 사용하여 `function` 키워드 생략 가능
* 함수는 변수에 담을 수 있는 **'값'** 이며, 이를 통해 함수 표현식 형태로 사용할 수 있다.
...
* `this`: 현재 함수를 실행하는 주체
* 일반 함수: 호출 방식에 따라 `this`가 바뀜
* 화살표 함수: `this`를 새로 생성하지 않고, 함수가 정의된 위치의 외부 스코프의 `this`를 그대로 사용 (고정됨)

<br>
<br>

# 🗓️ 2026-03-27 (금) ~ 2026-03-28 (토)
## 🧩 스프링부트 인증 인가 수업 복습
**실습 깃허브** : https://github.com/devjjune/spring-authentication-authorization-practice.git

**학습 내용** : 회원 도메인을 중심으로 인증 구조를 도입하고, apiKey 기반 인증으로 개선하며 전체 로그인 흐름을 구축

1. 회원(Member) 도메인을 도입하고 샘플 데이터를 생성한 뒤, 기존 도메인(Post, Comment)에 회원 정보를 연관시킴

2. 글 작성 시 회원 정보를 기반으로 인증 기능을 구현
   - 요청 파라미터로 username과 password를 전달하여 작성자 인증 수행
   - DTO에 작성자 정보를 포함하되, 필요한 정보(아이디, 이름)만 노출하여 최소 공개 원칙 적용
   - 비밀번호 불일치 상황을 처리하기 위해 커스텀 예외 클래스 도입

3. 인증 방식을 username/password에서 apiKey 기반으로 변경
   - apiKey는 무작위 문자열로 구성되어 있어 노출 시에도 상대적으로 안전하며, 인증 수단으로 활용 가능

4. 회원가입 및 로그인 기능을 구현하여 사용자 인증 흐름을 완성
