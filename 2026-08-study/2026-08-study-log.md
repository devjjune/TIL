...
# 🗓️ 2026-08-19 (수) ~ 2026-08-20 (목)
## 🧩 비관적 락, 낙관적 락, 네임드 락
>📌 참고 강의: 재고시스템으로 알아보는 동시성 이슈 해결방법 (최상용)   
>📌 Synchronized를 이용한 방식과 데이터베이스(MySQL)를 이용한 방식

### 1. `synchronized` 키워드

* `synchronized`는 자바에서 제공하는 키워드로, **인스턴스 메서드 선언부에 사용하면 해당 객체의 모니터 락을 획득한 스레드 하나만 해당 `synchronized` 영역에 접근할 수 있다.**  
  (모든 객체는 내부적으로 모니터 락을 가지며, 상호 배제를 통해 여러 스레드의 동시 접근을 막는다.)

* **단점 1:** `@Transactional`과 함께 사용할 경우 **DB 동시성 문제를 완전히 해결하지 못할 수 있다.** `@Transactional`을 사용하면 Spring 프록시가 트랜잭션을 관리하는데, 실제 `synchronized` 메서드가 종료되어 락이 해제된 뒤 트랜잭션이 `commit`될 수 있다. 따라서 **락 해제 ~ DB 반영 사이에 다른 스레드가 접근할 가능성이 있다.**
  
* **단점 2:** `synchronized`의 락은 **하나의 JVM(프로세스) 내부에서만 공유**된다. 따라서 서버가 여러 대이거나 애플리케이션 프로세스가 여러 개라면 서로의 락을 공유하지 못하므로 동시성을 보장할 수 없다.

### 2. 비관적 락 (Pessimistic Lock)
```java
public interface StockRepository extends JpaRepository<Stock, Long> {
    // DB에 접근하는 리포지토리 계층에 선언

    @Lock(LockModeType.PESSIMISTIC_WRITE) // SpringData JPA에서는 어노테이션으로 쉽게 락 구현 가능
    @Query("select s from Stock s where s.id = :id")
    Stock findByIdWithPessimisticLock(Long id); // 인터페이스라 구현은 없고 선언만
}
```
- 실제로 데이터에 Lock을 걸어서 정합성을 맞추는 방법
- Exclusive Lock을 걸면 다른 트랜잭션에서는 Lock이 해제되기 전까지 데이터를 가져갈 수 없다. 
- 충돌이 빈번하게 일어날 때 낙관적 락보다 성능이 좋다. 
- 데드락이 걸릴 수 있어 유의해야 한다. 

### 3. 낙관적 락 (Optimistic Lock)
- 실제로 Lock을 이용하진 않고 버전을 이용해서 정합성을 맞춘다.
- 데이터를 읽은 후 update를 수행할 때 현재 내가 읽은 버전이 맞는지 확인한다. 수정 사항이 발생했을 시 데이터를 다시 읽은 후 수행해야 한다. 
- 낙관적 락을 이용한 서비스 로직에서는 충돌이 발생할 경우를 대비해 별도의 재시도 로직을 작성해야 한다.
`OptimisticLockStockService`가 한 번의 재고 감소를 시도하고 버전 충돌 여부를 확인하는 역할이라면, `OptimisticLockStockFacade`는 충돌로 실패했을 때 일정 시간 대기 후 다시 호출하는 재시도 로직을 담당한다.
- 데이터 충돌이 잦은 환경에서는 재시도 비용이 커져 비효율적일 수 있는 반면, 비관적 락은 대기 시간이 발생해도 일관성 보장이 확실하다. 

> 비관적 락은 충돌을 미리 막고 다른 트랜잭션을 대기시키는 방식이라 별도의 재시도가 보통 필요 없고, 낙관적 락은 충돌을 허용한 뒤 충돌이 확인되면 예외로 실패시키는 방식이므로 작업을 완료하려면 재시도 로직이 필요하다.

### 4. 네임드 락 (Named Lock)
- 데이터 행이나 테이블에 락을 거는 게 아니라, 개발자가 지정한 '이름'을 잠근다. 
- 이름을 가진 락을 획득한 후 해제할 때까지 다른 세션이 이 락을 획득할 수 없음
- 트랜잭션 종료될 때 별도의 명령어로 직접 락을 해지하거나 선점 시간이 끝나야 함
- MySQL에서는 get-lock 명령어로 락 획득, release-lock으로 해제
